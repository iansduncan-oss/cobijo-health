...
def _og_image(kind, slug, state="CA"):
    """Per-page share card generated by scripts/gen_og_images.py (language-neutral). CA hospital + county
    pages get a unique card; anything else (including statute-driven states, whose per-page OG cards are a
    later increment) falls back to the site-wide og-image.png — a valid card, never a 404/wrong-hospital
    reference (a same-named CA slug would otherwise be served for an IL/NY/MD/WA page)."""
    if state == "CA" and kind == "hospital" and slug:
        return f"{BASE}/og/hospital/{slug}.png"
    if state == "CA" and kind == "county" and slug:
        return f"{BASE}/og/county/{slug}.png"
    return f"{BASE}/og-image.png"


def _print_qr(kind, slug, caption, state="CA"):
    """Print-only QR (hidden on screen via .print-qr; @media print reveals it). Encodes the page's
    English canonical URL so a paper handout posted on a clinic wall taps back to the site. Codes are
    pre-generated by scripts/gen_qr_codes.py and served from /qr/. A statute-driven state's codes live
    under a /qr/<state-ns>/ prefix (encoding the /<ns>/… canonical) so they can't collide with a CA slug."""
    prefix = "" if state == "CA" else f"{state.lower()}/"
    return (f'<div class="print-qr"><img src="/qr/{prefix}{kind}/{slug}.svg" width="132" height="132" alt="">'
...
def render_statutory_hospital(row, slug, index, lang="en"):
    ...
    out.append(f'<div class="card sharewrap"><h2>{_e(S["h_share_h"])}</h2><div class="share">'
               ...
               f'<button type="button" class="linkbtn alt" onclick="window.print()">🖨 {_e(S["h_print"])}</button>'
               f'<span class="copied" id="copied"></span></div></div>')
    out.append(_print_qr("hospital", slug, S["h_share_h"], state))
...
def render_statutory_county(county, index, lang="en", state="IL"):
    ...
    out.append(f'<div class="card sharewrap"><h2>{_e(S["h_share_h"])}</h2><div class="share">'
               ...
               f'<button type="button" class="linkbtn alt" onclick="window.print()">🖨 {_e(S["h_print"])}</button>'
               f'<span class="copied" id="copied"></span></div></div>')
    out.append(_print_qr("county", cslug, S["h_share_h"], state))
    out.append(_foot(S, lang))
    out.append(_share_js(_e(S["h_copied"])))
    return "".join(out)
