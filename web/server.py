...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path.startswith("/og/") and path.endswith(".png"):
            # Per-hospital / per-county OG share cards (pre-generated by scripts/gen_og_images.py).
            # Contain to web/og/ via realpath so a crafted ../ path can't escape the directory.
            og_root = os.path.realpath(os.path.join(HERE, "og"))
            target = os.path.realpath(os.path.join(HERE, path.lstrip("/")))
            if (target == og_root or target.startswith(og_root + os.sep)) and os.path.isfile(target):
